 ...
 * 文件搜搜模态框
 */
public class FileContextSearchModalView {
    ...
    public FileContextSearchModalView() {
    ...
    private class FileSearchView extends AbstractMigView {
        ...
        protected void init() {
            ...
            fileContextJList.setCellRenderer(new FileContextSearchRender());
        ...
        protected void render() {
            row = new JPanel(new MigLayout("wrap 1,gap 0,insets 0,h 600"));
            row.setBackground(ThemeColor.themeColor);
            row.add(search, "w 400:100%:,h 35!");
            row.add(fileListScroll, "grow,gapleft 2,gapright 2");
            fileListScroll.setVisible(false);
            JXTitledPanel titledPanel = SearchListStyleWrapper.createTitledPanel("Search Content", row);
